...
Range = Aura:CreateSlider({
	...
	Function = function(value)
		print('I love dih', value)
	end
})
Rotations = Aura:CreateToggle({
	Name = 'Rotations'
})
Sort = Aura:CreatePicker({
	Name = 'Sort',
	Options = {'8', '12'},
	Function = function(val)
		print(val..' inch dildo is entering '..lplr.Name)
	end
})
...
HUD = tabs.GUI:CreateModule({
	...
	Function = function(callback)
		if callback then
			...
			ArrayFrame.Visible = Arraylist.Enabled
		else
			lib.API.Watermark(callback, 'Monsoon')
			ArrayFrame.Visible = callback
		end
	end
})
